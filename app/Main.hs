    ...
    then do
      ...
      if opcao == "2"
        then do
          ...
          mapM_ (putStrLn . Auxiliar.printaConta) contas_usuario
                            ...
                            then putStr "\nTá querendo tirar dinheiro de onde não tem??\n"
                              ...
                              if null user
                                then putStrLn "\nUsuário não cadastrado(O que você pensa que está fazendo??)"
                                  ...
                                  let usuarios_modificados = Data.List.delete (head (Auxiliar.getUsuario usuarios login)) usuarios
                                  let usuario_com_transferencia = Usuario.realizaTransacao transferencia

                                  if Data.Maybe.isNothing usuario_com_transferencia
