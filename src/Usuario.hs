...
realizaTransacao :: Transferencia -> Maybe Usuario
realizaTransacao
  Transferencia
    { usuario = usuario,
    ...
    let novaContaOrigem = contaOrigem {saldoConta = saldoConta contaOrigem - valor}
     ...
     in if saldoConta novaContaOrigem < 0
          then Nothing
          else Just usuario {contas = alterarValoresContas (novaContaOrigem, novaContaDestino) (contas usuario)}
